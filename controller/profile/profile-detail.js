...
const user_detail = (req, res, next) => {
    let detailUser = 'select username,email,phone_number,role,birthFrom,avatar,avatar_ver from Account where userID = ?';
    ...
    form.parse(req, (err, fields, files) => {
        ...
        if (files.avatar.size > 0) {
          cloud.uploader.upload(
            ...
            (err, result) => {
              if (err) return next(err);
              let updateUser = "update Account set username=?,email=?,phone_number=?,birthFrom=?,avatar=?,avatar_ver=? where userID=?";
              let dataAvatar = `${req.session.userID}.${result.format}`;
              db.query(updateUser, [username, email, phone_number, birthFrom, dataAvatar,  result.version, userID], (err, data) => {
                if (err) return next(err);
