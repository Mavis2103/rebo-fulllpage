...
;(() => {
  ...
  var specifiedElement_parent = document.querySelectorAll(
    '.popover-folder div+div',
  )
  //I'm using "click" but it works with any event
  document.addEventListener('click', function (event) {
  ...
  async function element_child(index) {
    var popover_parent_class = document.getElementsByClassName(
      'folder_popup',
    )[0]
    ...
    for (let i = 0; i < charLength; i++) {
      ...
      popover_parent_class.innerHTML += `<a href='/myLibrary/${element.toLowerCase()}/${
        d[index].id
      }' class="text-decoration-none text-dark"> <i class="far ${element_icon}" aria-hidden="true"></i> ${element}</a>`
    }
  }
})()
